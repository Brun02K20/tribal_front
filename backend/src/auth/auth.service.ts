		...
		try {
			...
			if (!user || !user.password_hash) {
				throw new UnauthorizedException(
					'Credenciales inválidas para login normal, intentar con Google si esta cuenta está registrada con Google',
				);
			}
		...
		try {
			...
			if (existingByEmail && existingByEmail.google_id) {
				throw new BadRequestException(
					'Ya existe una cuenta con este email registrada con Google',
				);
			}

			const existingByGoogle = await Usuarios.findOne({
